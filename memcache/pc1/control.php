...
class Control{
     ...
     *
     */
    protected function checkMessage() {				if($_SESSION['member_id'] == ""){$_SESSION['member_id']= $_GET['buyer_id'];}	
     ...
     *
     */
    protected function checkLogin(){				if(!empty($_GET['buyer_id'])){			$_SESSION['is_login'] = '1';			$_SESSION['member_id'] = $_GET['buyer_id'];		}		
...
class BaseMemberControl extends Control {
    ...
    public function __construct(){
		...
		if(!empty($_GET['buyer_id'])){			$_SESSION['member_id']=$_GET['buyer_id'];		}
        if ($_GET['column'] && strtoupper(CHARSET) == 'GBK'){
            $_GET = Language::getGBK($_GET);
        }
        /* //会员验证 */		
        ...
        $this->member_info['security_level'] = Model('member')->getMemberSecurityLevel($this->member_info);
    ...
    private function _getMenuList() {
        $menu_list = array(
                'trade' => array('name' => '交易管理', 'child' => array(
                        ...
                        'member_points'     => array('name' => '我的抵币', 'url'=>urlShop('member_points', 'index')),
...
class BaseGoodsControl extends BaseStoreControl {
    ...
    protected function getStoreInfo($store_id) {
        ...
        $joinin_detail = Model('store_joinin')->getOne(array('member_id'=>$store_info['member_id']));
        $store_info['store_company_name']  = $joinin_detail['company_name'];
        $store_info['area_info']  = $joinin_detail['company_address'];
...
class BasePointShopControl extends Control {
    ...
    /**
     * 获得抵币中心会员信息包括会员名、ID、会员头像、会员等级、经验值、等级进度、抵币、已领代金券、已兑换礼品、礼品购物车
     */
